...
class WPForms_Frontend {
	...
	public function __construct() {
		...
		// Actions.
		//add_action( 'wpforms_wp_footer', [$this,'wpf_dev_event_after_submit'], 100 );
		add_action( 'wp_enqueue_scripts', [$this, 'bootstrap_css']);
		add_action( 'wp_enqueue_scripts', [$this, 'bootstrap_js']);
 ...
 */
	...
	public function bnzsa_form_modal(){
		$html = "
		<style>
		...
		*::after {
		...
		<div class='section full-height'>
			  ...
			  <label for='modal-btn' style='margin-top:-600px'>Ver resultado :) <i class='uil uil-expand-arrows'></i></label> 	
			  <div class='modal'>		
				  <div class='modal-wrap'>	
				  <div style='display:flex; justify-content:center; flex-wrap:wrap;'><p style='width:100%; text-align:center;'>Mucho ánimo ".$fields[2]['value']. "</p><br><img src='https://sanjuanboscosalamanca.salesianas.org/wp-content/uploads/2020/04/%C3%81nimo.jpg' alt='BNZSA mucho animo' width='200' height='90'> </div>
					</div>			          		
			  </div>	
		</div>";
	...
	public function bnzsa_form($fields){
		//BNZSA

		//Si el value del radio input es "dejame en paz" lo redireccionamos a la página de paz.
		if($fields[3]["value"] == "Déjame en paz"){
			?>
			<script type="text/javascript">
			   jQuery( function( $ ) {
				setTimeout(function(){
					window.location.replace(window.location.href+'paz');
				 }, 1200);
			...
			$confirmation_message = 'Redirigiendo hacia la paz! <br> <img src="https://blog.oxfamintermon.org/wp-content/uploads/2018/07/definicion-de-paz.jpg" alt="BNZSA mucho animo" width="200" height="90"> ';
			

			printf(
				'<div class="%s" id="wpforms-confirmation-%d">%s
				</div>',
				$class,
				absint( $form_data['id'] ),
			...
			//Si es "animo" sacamos un mensaje de ánimo junto a una foto.
			//No incluí un "modal" porque no existe la 
		}else{
			
			$confirmation_message = $this->bnzsa_form_modal();		
			printf(
				'<div class="%s" id="wpforms-confirmation-%d">%s
				</div>',
				$class,
